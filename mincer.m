...
- (void)startStream
{
	...
	else if ([video_device indexOfSelectedItem] > desktop_count)
	{
		...
		[desc appendFormat:@"avfvideosrc name=video_src do-stats=true device-index=%d ! ", device_idx];
	}
	else
	{
		key_interval = framerates[[framerate indexOfSelectedItem]] * 2;
		
		[desc appendFormat:@"avfvideosrc name=video_src do-stats=true device-index=%ld capture-screen=true capture-screen-cursor=true ! video/x-raw, framerate=%d/1 ! ", [video_device indexOfSelectedItem] , framerates[[framerate indexOfSelectedItem]]];
	}
	...
	{
		[desc appendFormat:@"voaacenc bitrate=%d ! audio/mpeg, channels=2 ! aacparse ! tee name=tee_aac ", [audio_bitrate intValue]];
	}
