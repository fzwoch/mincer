...
- (void)startStream
{
	...
	[desc appendFormat:@"videoscale method=lanczos envelope=1 ! queue max-size-bytes=0 ! video/x-raw, width=%d, height=%d ! ", resolutions[[resolution indexOfSelectedItem]].width, resolutions[[resolution indexOfSelectedItem]].height];
	...
	[desc appendFormat:@"audiomixer name=audio_mix ! faac bitrate=%d rate-control=ABR ! aacparse ! tee name=tee_aac ", audio_bitrates[[audio_bitrate intValue]] * 1000];
